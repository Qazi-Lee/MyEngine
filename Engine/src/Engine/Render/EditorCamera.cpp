...
namespace ENGINE
{
	...
	{
		if (Input::IsKeyPress(ME_KEY_LEFT_ALT))
		{
			const glm::vec2& mouse{ Input::GetMouseX(), Input::GetMouseY() };
			glm::vec2 delta = (mouse - m_InitialMousePosition) * 0.003f;
			m_InitialMousePosition = mouse;
			if (Input::IsKeyPress(ME_KEY_Q))
				ReSet();
			if (Input::IsMouseButtonPress(ME_MOUSE_BUTTON_1))
				MousePan(delta);
			else if (Input::IsMouseButtonPress(ME_MOUSE_BUTTON_2))
				MouseRotate(delta);
			else if (Input::IsMouseButtonPress(ME_MOUSE_BUTTON_3))
				MouseZoom(delta.y);
		}
